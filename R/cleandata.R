...
#' Individual year, month, and day columns make filtering the data difficult.  This function combines them into a single date object.  
#' Only to be used internally to the package.
...
#' @details Note that R date objects do not handle BCE dates well.  For example, they assume a year 0 exists, when in fact dates go from 1 BCE to 1 CE, with no 0 in between.  
#'  Therefore, date differences between BCE and CE will not be correct.  However, date ranges should still be accurately reflected.
#'  Not all entries have date details beyond year, in that case the date is January 1 of the appropriate year
#' @examples
...
#' The LOCATION_NAME column in the NOAA database starts out life including the country name, as well as other formatting issues.  
#' Use this function to strip out and reformat the region for convenient labeleing
#' @param loc_nm String containing original location name from the NOAA earthquake file
#' @return A string removing the country name, fixing spacing, and converting to Title Case
#' @examples
...
#' @param raw_data A data frame containing the raw NOAA earthquake data.  
#' The function assumes the data has been read in using \code{readr::read_delim} with the option \code{delim = '\\t'}
#' @return A data frame of cleaned data, ready to be used in the visualization functions included in this package
#' @details
